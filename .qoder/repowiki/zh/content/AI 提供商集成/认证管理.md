...
**所做更改**
- 新增作用域提供程序环境变量系统支持
- 增强 OPENHARNESS_*_API_KEY 优先级处理机制
- 改进认证解析机制，支持多层级凭据来源
- 增强配置管理功能，优化凭据存储优先级
...
**更新** 新增作用域提供程序环境变量系统，支持 OPENHARNESS_*_API_KEY 优先级处理，改进认证解析机制，增强配置管理功能。
...
CLI["CLI<br/>认证命令"]
ENV["环境变量系统<br/>OPENHARNESS_*_API_KEY"]
ENDSCOPE["作用域提供程序<br/>凭据存储"]
end
AM --> STORE
AM --> CFG
AM --> EXT
...
- 配置与解析（Settings）
  ...
  - **新增** 环境变量系统：支持 OPENHARNESS_*_API_KEY 优先级处理
  - **新增** 作用域提供程序：支持按档案和提供者的作用域凭据存储
- CLI 认证命令
...
**更新** 新增环境变量优先级处理和作用域提供程序存储机制，提供更灵活的凭据管理方式。
...
AM->>CFG : 解析活动档案与认证源
alt 环境变量优先
CFG->>ENV : 检查 OPENHARNESS_*_API_KEY
ENV-->>CFG : 返回最高优先级凭据
else 作用域提供程序
CFG->>SCOPE : 检查档案作用域凭据
SCOPE-->>CFG : 返回作用域匹配凭据
...
**更新** 新增作用域提供程序存储支持，允许按档案和提供者名称进行精细的凭据隔离。

```mermaid
flowchart TD
Start(["写入凭据"]) --> CheckScope{"作用域提供程序?"}
CheckScope --> |是| ScopeStore["按档案:提供者名称存储"]
CheckScope --> |否| CheckKeyring{"可用密钥环?"}
...
- 运行时解析：resolve_auth() 按当前档案解析凭据，支持外部刷新

**更新** 新增环境变量系统和作用域提供程序支持：

#### 环境变量系统
- 支持 OPENHARNESS_*_API_KEY 优先级处理
- 自动检测并解析对应提供者的环境变量
- 提供兼容性支持（同时支持 OPENHARNESS_*_API_KEY 和 *_API_KEY）

#### 作用域提供程序存储
- 支持按档案名称和提供者名称的精细凭据隔离
- 优先检查档案作用域凭据，然后降级到全局存储
- 增强多账户场景下的凭据管理能力
...
class EnvVarSystem {
+OPENHARNESS_ANTHROPIC_API_KEY : str
+OPENHARNESS_OPENAI_API_KEY : str
+OPENHARNESS_DASHSCOPE_API_KEY : str
+OPENHARNESS_MOONSHOT_API_KEY : str
+OPENHARNESS_GEMINI_API_KEY : str
+OPENHARNESS_MINIMAX_API_KEY : str
+OPENHARNESS_NVIDIA_API_KEY : str
+OPENHARNESS_MODELSCOPE_API_KEY : str
}
class ScopeProviderStorage {
+profile : credential_slot : str
+credential_storage_provider_name() str
}
Settings --> ProviderProfile : "使用"
Settings --> ResolvedAuth : "解析"
Settings --> EnvVarSystem : "支持"
Settings --> ScopeProviderStorage : "支持"
```

**图示来源**
- [src/openharness/config/settings.py:116-140](file://src/openharness/config/settings.py#L116-L140)
- [src/openharness/config/settings.py:534-800](file://src/openharness/config/settings.py#L534-L800)
- [src/openharness/config/settings.py:383-390](file://src/openharness/config/settings.py#L383-L390)
- [src/openharness/config/settings.py:816-828](file://src/openharness/config/settings.py#L816-L828)
...
**更新** 新增环境变量解析缓存和作用域存储优化：
- 环境变量解析结果缓存，避免重复解析
- 作用域提供程序存储采用懒加载机制
- 多层级凭据查找优化，减少不必要的存储访问
...
**更新** 新增环境变量和作用域存储相关故障排除：
- 环境变量优先级问题
  - 检查 OPENHARNESS_*_API_KEY 是否正确设置
  - 验证环境变量与认证源的匹配关系
  - 确认环境变量值的格式和有效性
- 作用域提供程序存储问题
  - 检查档案作用域凭据的命名格式
...
**更新** 新增的作用域提供程序环境变量系统显著增强了认证管理的灵活性和安全性，支持多层级凭据来源的优先级处理，为复杂的企业级应用场景提供了更好的支持。
...
- API Key
  ...
  - **新增** 支持 OPENHARNESS_*_API_KEY 环境变量优先级处理
- OAuth 设备码（Copilot）
...
- **新增** 环境变量管理最佳实践
  - 优先使用 OPENHARNESS_*_API_KEY 环境变量进行凭据注入
  - 确保环境变量名称与认证源正确匹配
  - 在 CI/CD 环境中使用加密的环境变量存储
- **新增** 作用域提供程序存储最佳实践
  - 为不同档案设置独立的凭据存储空间
  - 使用清晰的提供者名称标识符
  - 定期清理不再使用的档案作用域凭据
