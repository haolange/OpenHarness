...
- [test_config_cli.py](file://tests/test_entrypoints/test_config_cli.py)
</cite>

## 更新摘要
**所做更改**
- 新增 config 子命令部分，详细介绍程序化配置管理功能
- 更新核心组件章节，添加 config 子命令的详细说明
- 新增配置管理最佳实践和使用场景
- 更新故障排除指南，包含 config 子命令相关的常见问题
...
- 子命令：mcp、plugin、auth、provider、cron、autopilot、config（新增）
...
CONFIG["配置管理: config_app<br/>config/show, config/set"]
END
subgraph "个人代理 ohmo"
...
- 主命令与选项
  ...
  - 配置管理：config show（显示设置）、config set（设置配置项）
...
## 架构总览
下图展示从命令行到运行时的核心流程，涵盖非交互输出、会话恢复、权限检查、配置管理与 UI 渲染。
...
participant ConfigApp as "配置应用 (config_app)"
participant Runtime as "运行时/引擎"
participant UI as "UI 后端 (ui/app.py)"
participant Renderer as "终端渲染 (ui/output.py)"
User->>CLI : 执行 oh [config 子命令] [其他选项]
alt 配置管理
CLI->>ConfigApp : 解析 config 命令
ConfigApp->>ConfigApp : config_show/config_set 处理
ConfigApp-->>User : 显示或更新配置
else 干运行 (--dry-run)
...
### 6) 配置管理（新增）
- config show：显示当前解析后的设置 JSON
- config set：持久化设置项到 ~/.openharness/settings.json

配置管理特性：
- 支持嵌套键访问（如 web.resolution_mode）
- 类型安全的值转换（布尔、整数、浮点数、列表）
- 自动保存到配置文件
- 支持环境变量覆盖

配置项类型转换：
- 布尔值：接受 1/true/yes/on 和 0/false/no/off
- 整数值：数字字符串转换为整数
- 浮点数值：数字字符串转换为浮点数
- 列表值：逗号分隔的字符串转换为列表
...
Ready --> Output{"--output-format" + "配置管理"}
...
- 配置管理通过 config_app 子命令提供程序化配置接口
...
## 结论
OpenHarness CLI 提供了从交互式会话到非交互输出、从权限控制到系统上下文配置的完整能力。新增的 config 子命令进一步增强了程序化配置管理功能，使用户能够通过命令行直接查询和修改配置。通过合理选择输出格式、权限模式与推理参数，可在不同场景（开发、CI、自动化脚本）中高效使用。干运行预览是理解当前配置与潜在风险的重要工具，建议在执行前先运行 --dry-run。
...
- 系统与上下文
  ...
  - oh --theme dark
- 配置管理（新增）
  - oh config show：显示当前所有设置
  - oh config set web.resolution_mode synthetic_dns：设置网络解析模式
  - oh config set web.synthetic_dns_cidrs "100.64.0.0/10,203.0.113.0/24"：设置 DNS CIDR
  - oh config set sandbox.enabled true：启用沙箱
- 调试
  - oh --debug -p '测试'

### B. 配置管理最佳实践
- 使用 config show 查看当前配置状态
- 通过嵌套键精确设置配置项
- 批量更新配置以提高效率
- 使用环境变量进行配置覆盖
- 定期备份配置文件
