...
## 更新摘要
**所做更改**
- 新增网络守卫系统章节，详细介绍AUTO、DIRECT、PROXY、SYNTHETIC_DNS四种解析模式
- 更新网络访问控制部分，增加新的DNS解析模式和配置选项
- 新增安全配置示例，包含新的网络守卫配置参数
- 更新故障排查指南，增加网络守卫相关问题的解决方案
...
**更新** 新增网络守卫系统，包含AUTO、DIRECT、PROXY、SYNTHETIC_DNS四种解析模式，显著增强了网络安全控制能力。
...
- 沙箱与网络：路径边界校验、公网 HTTP 访问守卫（新增网络守卫系统）
...
J["network_guard.py<br/>网络守卫系统"]
K["ResolutionMode<br/>解析模式枚举"]
end
L["测试用例<br/>test_checker.py / test_path_validator.py / test_network_guard.py"]
A --> C
A --> B
D --> A
E --> A
...
- [network_guard.py:37-44](file://src/openharness/utils/network_guard.py#L37-L44)
...
- [network_guard.py:1-341](file://src/openharness/utils/network_guard.py#L1-L341)
...
- 公网 HTTP 守卫（network_guard）：校验 URL 协议、主机、凭据字段，并拒绝私有/回环地址目标。**新增** 包含四种解析模式：AUTO、DIRECT、PROXY、SYNTHETIC_DNS。
...
- **新增** 网络层：网络守卫系统提供多种解析模式，确保 HTTP 请求的安全性和合规性。
...
participant NetGuard as "网络守卫系统"
...
opt 网络访问
Agent->>NetGuard : 验证HTTP请求
NetGuard-->>Agent : 返回解析结果
end
```
...
- [network_guard.py:130-186](file://src/openharness/utils/network_guard.py#L130-L186)
...
- 辅助函数：路径匹配扩展：为目录工具提供"目录根"与"目录根/"两种形式参与匹配。
...
- **新增** 解析模式：包含 AUTO、DIRECT、PROXY、SYNTHETIC_DNS 四种模式，提供灵活的 DNS 解析策略。
...
### 网络守卫系统详解
**新增** OpenHarness 现在网络守卫系统提供了四种解析模式，每种模式都有特定的安全策略：

#### 解析模式枚举（ResolutionMode）
- AUTO：自动模式，根据是否存在代理自动选择 DIRECT 或 PROXY 模式
- DIRECT：直接模式，严格验证目标地址必须为公网地址
- PROXY：代理模式，通过代理服务器解析目标，不进行本地 DNS 查询
- SYNTHETIC_DNS：合成 DNS 模式，允许用户声明的 CIDR 网络范围内的私有地址

#### DIRECT 模式
- 严格验证目标必须为公网地址
- 通过 DNS 解析获取目标 IP 地址
- 拒绝所有非全局单播地址（包括回环、私有网络、链路本地等）

#### PROXY 模式
- 通过配置的代理服务器解析目标
- 不进行本地 DNS 查询，避免泄露内部网络信息
- 仅验证 URL 语法和主机名合法性

#### SYNTHETIC_DNS 模式
- 允许用户声明的 CIDR 网络范围内的私有地址
- 需要配合 web.synthetic_dns_cidrs 配置使用
- 适用于使用合成 DNS 的企业网络环境
...
```mermaid
flowchart TD
Start(["网络请求开始"]) --> Mode{"解析模式"}
Mode --> AUTO{"AUTO 模式?"}
AUTO --> |是| ProxyCheck{"是否有代理?"}
ProxyCheck --> |是| PROXY["PROXY 模式"]
ProxyCheck --> |否| DIRECT["DIRECT 模式"]
AUTO --> |否| ModeSelect{"选择指定模式"}
ModeSelect --> DIRECT
ModeSelect --> PROXY
ModeSelect --> SYNTHETIC["SYNTHETIC_DNS 模式"]
DIRECT --> DirectCheck["检查目标是否为公网地址"]
DirectCheck --> |是| Allow1["允许访问"]
DirectCheck --> |否| Deny1["拒绝访问"]
PROXY --> ProxyCheck2["验证 URL 语法和主机名"]
ProxyCheck2 --> Allow2["允许访问"]
SYNTHETIC --> CIDRCheck["检查是否在声明的 CIDR 范围内"]
CIDRCheck --> |是| Allow3["允许访问"]
CIDRCheck --> |否| Deny3["拒绝访问"]
```

**图表来源**
- [network_guard.py:61-79](file://src/openharness/utils/network_guard.py#L61-L79)
- [network_guard.py:114-127](file://src/openharness/utils/network_guard.py#L114-L127)
- [network_guard.py:225-255](file://src/openharness/utils/network_guard.py#L225-L255)
...
- **新增** 网络守卫依赖：
  - httpx 异步 HTTP 客户端
  - ipaddress IPv4/IPv6 地址处理
  - socket DNS 解析
  - 环境变量配置（OPENHARNESS_WEB_PROXY、OPENHARNESS_WEB_RESOLUTION_MODE、OPENHARNESS_WEB_SYNTHETIC_DNS_CIDRS）
...
- [network_guard.py:11](file://src/openharness/utils/network_guard.py#L11)
- [network_guard.py:157-162](file://src/openharness/utils/network_guard.py#L157-L162)
...
- 网络守卫：
  ...
  - **新增** 不同解析模式的性能差异：DIRECT 模式需要 DNS 解析，PROXY 模式不需要本地解析，SYNTHETIC_DNS 模式需要 CIDR 匹配。
- 沙箱路径校验：
...
- **新增** 网络守卫相关问题：
  - 解析模式配置错误：检查 web.resolution_mode 是否为 AUTO/DIRECT/PROXY/SYNTHETIC_DNS 之一。
  - PROXY 模式失败：确认代理服务器配置正确，且代理 URL 符合语法要求。
  - SYNTHETIC_DNS 模式失败：检查 web.synthetic_dns_cidrs 配置是否正确，确保目标地址在声明的 CIDR 范围内。
  - DNS 解析失败：检查网络连接和 DNS 服务器配置。
...
- [network_guard.py:25-96](file://src/openharness/utils/network_guard.py#L25-L96)
- [test_network_guard.py:44-141](file://tests/test_utils/test_network_guard.py#L44-L141)

## 结论
OpenHarness 的权限与安全体系通过"内置敏感路径保护 + 模式化策略 + 规则化路径/命令控制 + 交互确认 + 多智能体协作 + 沙箱边界与网络守卫"的组合，实现了从单机到多智能体场景下的可控执行。默认模式兼顾安全与可用性，计划模式适合审阅阶段，自动模式适用于受控实验环境。

**更新** 新增的网络守卫系统显著增强了网络安全控制能力，四种解析模式（AUTO、DIRECT、PROXY、SYNTHETIC_DNS）提供了灵活的 DNS 解析策略，能够适应各种网络环境和安全需求。建议在生产环境中严格限制自动模式使用，合理配置路径与命令规则，并启用交互确认与沙箱边界控制。对于网络访问，应根据实际网络环境选择合适的解析模式，并正确配置相关的安全参数。
...
- 公网 HTTP 访问
  ...
  - **新增** 配置解析模式：web.resolution_mode（AUTO/DIRECT/PROXY/SYNTHETIC_DNS）
  - **新增** 配置代理服务器：web.proxy（HTTP/HTTPS 代理 URL）
  - **新增** 配置合成 DNS CIDR：web.synthetic_dns_cidrs（逗号分隔的 CIDR 列表）
...
- 沙箱与网络
  ...
  - **新增** 根据网络环境选择合适的解析模式：内网环境推荐 PROXY 或 SYNTHETIC_DNS，公网环境推荐 DIRECT。
  - **新增** 正确配置代理服务器和合成 DNS 参数，避免安全漏洞。
- 多智能体协作
  - 在团队目录中统一管理权限请求；领导者集中审批并应用规则更新；定期清理旧的已决请求文件。
- **新增** 网络守卫最佳实践
  - PROXY 模式：适用于需要通过代理访问互联网的环境，确保代理服务器的安全性。
  - DIRECT 模式：适用于可以直接访问公网的环境，提供最严格的地址验证。
  - SYNTHETIC_DNS 模式：适用于使用合成 DNS 的企业网络，必须严格配置允许的 CIDR 范围。
  - AUTO 模式：简化配置，但需要确保代理配置的正确性。